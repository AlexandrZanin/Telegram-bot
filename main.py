...
"""
Основной скрипт запускает работу,
в нем нужные импорты и функции, которые используют декораторы
message_handler и callback.
"""
...
commands_list = ['/lowprice - топ самых дешёвых отелей',
...
def history_message(message: types.Message):
    if orm.Guest.select():
        counter = 0
        for id_request in orm.Guest.select().where(orm.Guest.user_id == message.from_user.id).order_by(
                orm.Guest.time_now.desc()):
            ...
            if id_request.cmd == '/bestdeal':
                ...
                text_button = "Время запроса: {time_now}\nКоманда {cmd}\nГород: {sity}\nДата заезда: {in_date}" \
                            "\nДата выезда: {out_date}".format(time_now=id_request.time_now[:-7],
                                                               cmd=id_request.cmd, sity=id_request.sity,
                                                               in_date=id_request.date_in,
        ...
        bot.send_message(message.chat.id, text="Последние {} запрос{}".format(counter, ending),
...
def callback_worker(query):
    if query.data == 'again':
        bot.send_message(query.message.chat.id, text='Выберите город для поиска отелей',
                         reply_markup=types.ReplyKeyboardRemove())
...
def cal(c):
    ...
    if not user.date_in:
        result, key, step = MyStyleCalendar(min_date=date.today(), max_date=date(2023, 12, 31), locale='ru').process(
            c.data)
        ...
        result, key, step = MyStyleCalendar(min_date=user.date_in + timedelta(days=+1),
...
def get_max_dist(message: types.Message):
    user = User.get_user(message.chat.id)
    if re.findall('\A\d+\Z', message.text) or re.findall('^\d+[.,]+\d+', message.text):
        # соответствует ли шаблону число или дробное число
